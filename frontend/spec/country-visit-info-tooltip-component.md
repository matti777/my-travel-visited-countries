...
Omit any section that has nothing to show. If notes, tags, and `MediaURL` are all absent, do not attach a tooltip.

1. **Notes** — when the visit has non-empty notes: render as Markdown (sanitized HTML). Place this block at the **top** of the tooltip with a **distinct background** from the rest of the tooltip body.
